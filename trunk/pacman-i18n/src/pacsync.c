...
int sync_synctree()
{
	...
	for(i = pmc_syncs; i; i = i->next) {
		...
		vprint(_("pacsync: new mtime for %s: %s\n"), sync->treename, newmtime);
		FREELIST(files);
		if(ret > 0) {
			fprintf(stderr, _("failed to synchronize %s\n"), sync->treename);
			success = 0;
		} else if(ret < 0) {
			...
			if(success) {
				...
				/* remove the old dir */
				vprint(_("removing %s (if it exists)\n"), ldir);
				rmrf(ldir);
	...
	for(i = servers; i && !done; i = i->next) {
		...
		if(!pmo_xfercommand && strcmp(server->protocol, "file")) {
			if(!strcmp(server->protocol, "ftp") && !pmo_proxyhost) {
				...
				if(!FtpConnect(server->server, &control)) {
					fprintf(stderr, _("error: cannot connect to %s\n"), server->server);
					continue;
				}
				...
				if(!FtpChdir(server->path, control)) {
					fprintf(stderr, _("error: could not cwd to %s: %s\n"), server->path,
		...
		for(lp = files; lp; lp = lp->next) {
			...
			if(pmo_xfercommand && strcmp(server->protocol, "file")) {
				...
				getcwd(cwd, PATH_MAX);
				if(chdir(localpath)) {
					fprintf(stderr, _("error: could not chdir to %s\n"), localpath);
					return(1);
				}
				...
				if(!strcmp(server->protocol, "ftp") && !pmo_proxyhost) {
					if(!FtpSize(fn, &fsz, FTPLIB_IMAGE, control)) {
						fprintf(stderr, _("warning: failed to get filesize for %s\n"), fn);
					}
					/* check mtimes */
					if(mtime1 || mtime2) {
						char fmtime[64];
						if(!FtpModDate(fn, fmtime, sizeof(fmtime)-1, control)) {
							fprintf(stderr, _("warning: failed to get mtime for %s\n"), fn);
						} else {
							trim(fmtime);
					...
					if(copyfile(src, output)) {
						fprintf(stderr, _("failed copying %s\n"), src);
					} else {
						filedone = 1;
					}
				}
