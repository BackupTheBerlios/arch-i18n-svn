...
int main(int argc, char *argv[])
{
	...
	if(pmo_op != PM_MAIN && pmo_op != PM_QUERY && pmo_op != PM_DEPTEST) {
		if(pmo_op == PM_SYNC && !pmo_s_sync &&
			...
			if(lckmk(lckfile, 1, 1) == -1) {
				fprintf(stderr, _("error: unable to lock pacman database.\n"));		
	...
	if(pmo_logfile && myuid == 0) {
		...
		if(logfd == NULL) {
			perror(_("warning: cannot open logfile"));
		}
	}
	...
	vprint(_("Installation Root: %s\n"), pmo_root);
	/* db location */
	vprint(_("Top-level DB Path: %s%s\n"), pmo_root, pmo_dbpath);
	if(pmo_verbose) {
	...
	if(db_local == NULL) {
		...
		vprint(_("initializing database %s...\n"), path);
		ret = makepath(path);
	...
	switch(pmo_op) {
		...
		default: fprintf(stderr, _("error: no operation specified (use -h for help)\n\n"));
						 ret = 1;
	}
	db_close(db_local);
	cleanup(ret);
	...
	if(deps) {
		...
		for(lp = deps; lp; lp = lp->next) {
			...
			if(miss->type == CONFLICT) {
				...
				if(!pmo_d_resolve) {
					printf(_("requires: %s"), miss->depend.name);
...
int pacman_sync(pacdb_t *db, PMList *targets)
{
	...
	if(pmo_s_clean) {
		if(pmo_s_clean == 1) {
			...
			if(rmrf(path)) {
				fprintf(stderr, _("error: could not remove cache directory: %s\n"), strerror(errno));
				return(1);
			}

			oldmask = umask(0000);
			if(makepath(path)) {
				fprintf(stderr, _("error: could not create new cache directory: %s\n"), strerror(errno));
				return(1);
			}
			umask(oldmask);
		}
	...
	if(pmo_s_sync) {
		...
		logaction(NULL, _("synchronizing package lists"));
		sync_synctree();
	}
	...
	for(i = pmc_syncs; i; i = i->next) {
		...
		if(db_sync == NULL) {
			...
			fprintf(stderr, _("       have you used --refresh yet?\n"));
			return(1);
		}
	...
	if(pmo_s_search) {
		...
		for(i = pkgs; i; i = i->next) {
			...
			if(!found) {
				fprintf(stderr, _("Package \"%s\" was not found.\n"), (char *)i->data);
				allgood = 0;
				break;
			}
		}
		...
		for(i = reps; i; i = i->next) {
			...
			if(!found) {
				printf(_("Repository \"%s\" was not found.\n"), (char *)i->data);
				allgood = 0;
				break;
			}
		}
		...
		logaction(NULL, _("starting full system upgrade"));
					...
					for(m = pm_packages; m; m = m->next) {
						...
						if(!strcmp(k->data, p->name)) {
							if(is_in(p->name, pmo_ignorepkg) || is_in(p->name, pmo_s_ignore)) {
								...
								ignore = 1;
							} else if(yesno(_(":: Replace %s with %s from \"%s\"? [Y/n] "), p->name, pkg->name, dbs->db->treename)) {
		...
		for(i = pm_packages; i && allgood; i = i->next) {
			...
			if(cmp > 0 && !sync->pkg->force) {
				...
				fprintf(stderr, _(":: %s: local version (%s) is newer than repo version (%s)\n"),
		...
		if((newer || ignore) && allgood) {
			fprintf(stderr, _(":: Above packages will be skipped.  To manually upgrade use 'pacman -S <pkg>'\n"));
		}
			...
			if(i->data) {
					...
					if(treename == NULL) {
						...
						if(k != NULL) {
							...
							if(yesno(_("    Install whole content? [Y/n] "))) {
								...
								PMList *l;
								for(l = k; l; l = l->next) {
									if(yesno(_(":: Install %s from group %s? [Y/n] "), (char*)l->data, targ)) {
							...
							fprintf(stderr, _("%s: not found in sync db\n"), targ);
							allgood = 0;
						}
					} else {
						fprintf(stderr, _("%s: not present in \"%s\" repository\n"), targ, treename);
						allgood = 0;
					}
					FREE(sync);
					FREE(targline);
				...
				if(local && !pmo_s_downloadonly && !pmo_s_printuris) {
					...
					if(cmp > 0) {
						...
						if(!yesno(_(":: %s-%s: local version is newer.  Upgrade anyway? [Y/n] "), local->name, local->version)) {
							FREEPKG(local);
							FREEPKG(sync->pkg);
							FREE(sync);
							FREE(targline);
						...
						/* versions are identical */
						if(!yesno(_(":: %s-%s: is up to date.  Upgrade anyway? [Y/n] "), local->name, local->version)) {
							FREEPKG(local);
							FREEPKG(sync->pkg);
							FREE(sync);
							FREE(targline);
			...
			if(deps) {
				for(i = deps; i; i = i->next) {
					...
					if(miss->type == DEPEND || miss->type == REQUIRED) {
						...
						fprintf(stderr, _("  %s: requires %s"), miss->target, miss->depend.name);
						switch(miss->depend.mod) {
						...
						if(!found && !is_in(miss->depend.name, rmtargs)) {
							...
							if(!solved) {
								...
								if(is_pkgin(&p1, pm_packages)) {
									if(yesno(_(":: %s conflicts with %s. Remove %s? [Y/n] "),
				...
				if(current && !strcmp(current->treename, sync->dbs->sync->treename)) {
					...
					if(pmo_s_printuris) {
						...
						if(stat(path, &buf)) {
							...
							vprint(_(" %s-%s%s is already in the cache\n"), sync->pkg->name, sync->pkg->version, PKGEXT);
							count++;
						}
					}
				}
			...
			if(files) {
				if(pmo_s_printuris) {
					...
					if(stat(ldir, &buf)) {
						...
						logaction(stderr, _("warning: no %s cache exists.  creating..."), ldir);
						oldmask = umask(0000);
						if(makepath(ldir)) {				
							/* couldn't mkdir the cache directory, so fall back to /tmp and unlink
							 * the package afterwards.
							 */
							logaction(stderr, _("warning: couldn't create package cache, using /tmp instead"));
					...
					if(downloadfiles(current->servers, ldir, files)) {
						fprintf(stderr, _("error: failed to retrieve some files from %s\n"), current->treename);
						allgood = 0;
					}
				}
		...
		if(allgood) {
			...
			for(i = final; i; i = i->next) {
				...
				if(md5sum1 == NULL || md5sum1[0] == '\0') {
					...
					fprintf(stderr, _("error: can't get md5 checksum for package %s\n"), pkgname);
					allgood = 0;
					continue;
				}
				...
				if(md5sum2 == NULL || md5sum2[0] == '\0') {
					...
					fprintf(stderr, _("error: can't get md5 checksum for archive %s\n"), pkgname);
					allgood = 0;
					continue;
				}
				...
				if(strcmp(md5sum1, md5sum2) != 0) {
					...
					fprintf(stderr, _("error: archive %s is corrupted\n"), pkgname);
					allgood = 0;
				}
...
int pacman_add(pacdb_t *db, PMList *targets, PMList *dependonly)
{
		...
		if(strstr(targ->data, "://")) {
			...
			*path = '\0';
			...
			if(downloadfiles(servers, ".", files)) {
				fprintf(stderr, _("error: failed to download %s\n"), (char*)targ->data);
				return(1);
			}
			FREELIST(servers);
			files->data = NULL;
		...
		if(lp) {
			...
			for(j = lp; j; j = j->next) {
				...
				if(miss->type == DEPEND || miss->type == REQUIRED) {
					...
					printf(_("  %s: requires %s"), miss->target, miss->depend.name);
					switch(miss->depend.mod) {
						case DEP_EQ: printf("=%s", miss->depend.version);  break;
						case DEP_GE: printf(">=%s", miss->depend.version); break;
						case DEP_LE: printf("<=%s", miss->depend.version); break;
	...
	for(targ = alltargs; targ; targ = targ->next) {
		...
		if(!pmo_upgrade && is_pkgin(info, pm_packages)) {
			fprintf(stderr, _("error: %s is already installed. (try --upgrade)\n"), info->name);
			continue;
		}
		...
		if(pmo_upgrade) {
			if(is_pkgin(info, pm_packages)) {
				...
				printf(_("upgrading %s... "), info->name);
				neednl = 1;
		...
		if(!pmo_upgrade) {
			printf(_("installing %s... "), info->name);
			neednl = 1;
		...
		for(i = 0; !th_read(tar); i++) {
			...
			if(nb) {
				...
				if(tar_extract_file(tar, temp)) {
					logaction(stderr, _("could not extract %s: %s"), pathname, strerror(errno));
					errors++;
					continue;
				}
				md5_pkg = MDFile(temp);
				...
				vprint(_("checking md5 hashes for %s\n"), expath);
				vprint(_("  current:  %s\n"), md5_local);
				vprint(_("  new:      %s\n"), md5_pkg);
				if(md5_orig) {
					vprint(_("  original: %s\n"), md5_orig);
				...
				if(!pmo_upgrade) {
					...
					if(strcmp(md5_local, md5_pkg)) {
						...
						if(rename(expath, newpath)) {
							logaction(stderr, _("error: could not rename %s: %s"), expath, strerror(errno));
						}
						if(copyfile(temp, expath)) {
							logaction(stderr, _("error: could not copy %s to %s: %s"), temp, expath, strerror(errno));
							errors++;
						} else {
							logaction(stderr, _("warning: %s saved as %s"), expath, newpath);
						}
					}
				} else if(md5_orig) {					
					/* PM_UPGRADE */
					...
					if(!strcmp(md5_orig, md5_local)) {
						...
						char newpath[PATH_MAX];
						vprint(_("  action: saving current file and installing new one\n"));
						installnew = 1;
					...
					if(installnew) {
						/*vprint("  %s\n", expath);*/
						if(copyfile(temp, expath)) {
							fprintf(stderr, _("error: could not copy %s to %s: %s\n"), temp, expath, strerror(errno));
							errors++;
						}
					}
				}
				...
				if(!notouch) {
					...
					vprint(_("%s is in NoUpgrade - skipping\n"), pathname);
					strncat(expath, ".pacnew", PATH_MAX);
					logaction(stderr, _("warning: extracting %s%s as %s"), pmo_root, pathname, expath);
					/*tar_skip_regfile(tar);*/
				}
				if(pmo_force) {
		...
		if(errors) {
			ret = 1;
			logaction(stderr, _("errors occurred while %s %s"),
 ...
 */
			...
			if(db_write(db, info, INFRQ_ALL)) {
				logaction(stderr, _("error updating database for %s!"), info->name);
				return(1);
			}
			vprint(_("done.\n"));
			if(pmo_upgrade && oldpkg) {
				logaction(NULL, _("upgraded %s (%s -> %s)"), info->name,
						oldpkg->version, info->version);
			} else {
				logaction(NULL, _("installed %s (%s)"), info->name, info->version);
			}
	...
	for(lp = targets; lp; lp = lp->next) {
		...
		if(info == NULL) {
			...
			if(is_in((char *)lp->data, groups)) {
				...
				printf(_(":: group %s:\n"), (char*)lp->data);
				list_display("   ", pkgs);
				if(yesno(_("    Remove whole content? [Y/n] "))) {
					...
					for(j = pkgs; j; j = j->next) {
						if(yesno(_(":: Remove %s from group %s? [Y/n] "), (char*)j->data, (char*)lp->data)) {
							info = db_scan(db, (char *)j->data, INFRQ_ALL);
							alltargs = list_add(alltargs, info);
						}
					}
			...
			FREELIST(groups);
			fprintf(stderr, _("error: could not find %s in database\n"), (char*)lp->data);
			return(1);
		}
		if(pmo_op == PM_REMOVE) {
			...
			for(j = pmo_holdpkg; j && j->data; j = j->next) {
				if(!strcmp(info->name, j->data)) {
					if(!yesno(_(":: %s is designated as a HoldPkg.  Remove anyway? [Y/n] "), info->name)) {
						return(1);
					}
					break;
				}
	...
	for(targ = alltargs; targ; targ = targ->next) {
		...
		if(!pmo_r_dbonly) {
			...
			for(lp = list_last(info->files); lp; lp = lp->prev) {
				...
				if(lstat(line, &buf)) {
					vprint(_("file %s does not exist\n"), line);
					continue;
				}
				if(S_ISDIR(buf.st_mode)) {
					/*vprint(_("  removing directory %s\n"), line);*/
					if(rmdir(line)) {
					...
					if(nb) {
						if(pmo_upgrade) {
							...
							if(!pmo_nosave) {
								...
								rename(line, newpath);
								logaction(stderr, _("warning: %s saved as %s"), line, newpath);
							} else {
								/*vprint(_("  unlinking %s\n"), line);*/
								if(unlink(line)) {
									perror(_("cannot remove file"));
								}
							}
						}
					} else {
		...
		if(!pmo_upgrade) {
			printf(_("done.\n"));
			logaction(NULL, _("removed %s (%s)"), info->name, info->version);
		}
	}
...
int pacman_query(pacdb_t *db, PMList *targets)
{
	...
	for(targ = targets; !done; targ = (targ ? targ->next : NULL)) {
		...
		if(pmo_group) {
			...
			if(package == NULL) {
				...
				if(!is_in(package, groups)) {
					fprintf(stderr, _("Group \"%s\" was not found.\n"), package);
					FREELIST(groups);
					return(2);
				}
		...
		if(pmo_q_isfile) {
			...
			if(info == NULL) {
				fprintf(stderr, _("error: %s is not a package\n"), package);
				return(1);
			}
			if(pmo_q_info) {
				dump_pkg_full(info);
		...
		if(pmo_q_owns) {
			...
			if(realpath(package, rpath)) {
				...
				if(!gotcha) {
					fprintf(stderr, _("No package owns %s\n"), package);
				}
				continue;
			} else {
				fprintf(stderr, _("error: %s is not a file.\n"), package);
				return(1);
			}
		}
						...
						for(lp = info->backup; lp; lp = lp->next) {
							...
							*ptr = '\0';
							...
							if(!stat(path, &buf)) {
								md5sum = MDFile(path);
								if(md5sum == NULL) {
									fprintf(stderr, _("error calculating md5sum for %s\n"), path);
									continue;
								}
	...
	for(i = targs; i; i = i->next) {
		...
		for(j = pkg->depends; j; j = j->next) {
			...
			if(dep == NULL) {
				...
				if(k == NULL) {
					fprintf(stderr, _("warning: cannot find package \"%s\" or anything that provides it!\n"), depend.name);
					continue;
				}
			...
			if(dep->reason == REASON_EXPLICIT) {
				vprint(_("excluding %s -- explicitly installed\n"), dep->name);
				needed = 1;
			}
...
int resolvedeps(pacdb_t *local, PMList *databases, syncpkg_t *syncpkg, PMList *list, PMList *trail)
{
	...
	for(i = deps; i; i = i->next) {
		...
		if(miss->type == DEPEND) {
			...
			if(!found) {
				fprintf(stderr, _("error: cannot resolve dependencies for \"%s\":\n"), miss->target);
				fprintf(stderr, _("       \"%s\" is not in the package set\n"), miss->depend.name);
				return(1);
			}
			found = 0;
			...
			if(!found) {
				...
				if(found) {
					usedep = yesno(_("%s requires %s, but it is in IgnorePkg.  Install anyway? [Y/n] "),
				...
				vprint(_("dependency cycle detected: %s\n"), sync->pkg->name);
				FREEPKG(sync->pkg);
				FREE(sync);
			}
		}
		...
		for(i = targets; i; i = i->next) {
			...
			for(j = tp->depends; j; j = j->next) {
				...
				if(splitdep((char*)j->data, &depend)) {
					logaction(stderr, _("warning: invalid dependency in %s"), (char*)tp->name);
					continue;
				}
				found = 0;
...
int runscriptlet(char *installfn, char *script, char *ver, char *oldver)
{
	...
	if(!grep(scriptfn, script)) {
		...
		if(strlen(tmpdir) && rmrf(tmpdir)) {
			fprintf(stderr, _("warning: could not remove tmpdir %s\n"), tmpdir);
		}
		return(0);
	}
...
int parseconfig(char *configfile)
{
	...
	while(fgets(line, PATH_MAX, fp)) {
		...
		if(line[0] == '[' && line[strlen(line)-1] == ']') {
			...
			if(!strlen(section)) {
				fprintf(stderr, _("config: line %d: bad section name\n"), linenum);
				return(1);
			}
			...
			key = strsep(&ptr, "=");
			if(key == NULL) {
				fprintf(stderr, _("config: line %d: syntax error\n"), linenum);
				return(1);
			}
			trim(key);
			key = strtoupper(key);
			if(!strlen(section) && strcmp(key, "INCLUDE")) {
				fprintf(stderr, _("config: line %d: all directives must belong to a section\n"), linenum);
				return(1);
			}
			if(ptr == NULL) {
				...
				if(!strcmp(key, "INCLUDE")) {
					...
					if(!strcmp(key, "NOUPGRADE")) {
						...
						if(p) {
							p += 3;
							if(p == NULL || *p == '\0') {
								fprintf(stderr, _("config: line %d: bad server location\n"), linenum);
								return(1);
							}
							ptr = p;
						}
					...
					if(!strcmp(key, "SERVER")) {
						...
						*p = '\0';
						...
						if(!strcmp(server->protocol, "ftp") || !strcmp(server->protocol, "http")) {
							...
							fprintf(stderr, _("config: line %d: protocol %s is not supported\n"), linenum, ptr);
							return(1);
						}
						/* add to the list */
...
void cleanup(int signum)
{
	...
	if(pm_access == READ_WRITE && lckrm(lckfile)) {
		logaction(stderr, _("warning: could not remove lock file %s"), lckfile);
	}
	if(workfile) {
